                ...
                for u, p in self.vault.sw_creds:
                    ...
                    try:
                        ...
                        if "auth" in error_str or "password" in error_str or "login" in error_str:
                            ...
                            print(f"    [X] Network Error to Switch {self.current_ip}: {str(e).strip().splitlines()[-1]}")
            ...
            while not auth_success:
                for u, p in self.vault.pa_creds:
                    ...
                    try:
                        ...
                        output = ""
                        # הגדלנו את זמני ההמתנה ל-15 שניות כדי לתת לפאלו להתאושש
                        for _ in range(30):
                            if shell.recv_ready():
                ...
                if not auth_success:
                    ...
                    if Utils.ask_yes_no(f"    Enter new credentials for this Firewall? (y/n): "):
                        ...
                        continue # ממשיך את הלולאה כמו בקוד הישן והיציב
                    else:
...
class NexusTracerApp:
    ...
    def identify_vendor(self, ip):
        ...
        for u, p in creds:
            try:
                client.connect(ip, username=u, password=p, timeout=8, look_for_keys=False, allow_agent=False)
                shell = client.invoke_shell()
                ...
                for _ in range(6): 
                    ...
                    if shell.recv_ready():
                        ...
                        if "sw-version" in out or "syntax" in out or "unknown" in out or "not found" in out:
                            break
                ...
                if "sw-version" in out or "family" in out or "mac-address" in out or "palo alto" in out:
                    return "PA"
                elif "syntax error" in out or "unknown command" in out or "not found" in out or "invalid" in out:
                    return "CP"
    ...
    def run_autonomous_discovery(self):
        ...
        for ip in ips:
            ...
            vendor = self.identify_vendor(ip)
